...
export class ContainerKingdomRenderer
{
  ...
  drawNetworks() {
    ...
    roadPlan.tiles.forEach((tile) => {
      const networkNames = [...tile.networks].sort();
  ...
  static buildNetworksRoadPlan(networks, cellWidth, cellHeight, roadWidth, roadHeight) {
    ...
    Object.keys(networks).sort().forEach((networkName) => {
      const connectedNodes = networks[networkName]
        .map((container) => {
          const coords = container?.rpgEngine?.data?.coords;
          ...
          return {
            id: container?.getId?.() ?? container?.Id ?? '',
            x: coords.x,
            y: coords.y,
          };
        })
      ...
      ContainerKingdomRenderer.buildNetworkTopology(connectedNodes).forEach(({ from, to }) => {
  ...
  /**
   * Build a deterministic spanning tree from a set of positioned containers.
   * Prim's algorithm keeps the shape stable and ignores the original API order.
   * @param {Array<{id: string, x: number, y: number}>} connectedNodes
   * @returns {Array<{from: {id: string, x: number, y: number}, to: {id: string, x: number, y: number}}>} the chosen edges
   */
  static buildNetworkTopology(connectedNodes) {
    if (connectedNodes.length <= 1) {
      return [];
    }

    const orderedNodes = [...connectedNodes].sort((left, right) =>
      ContainerKingdomRenderer.compareNetworkNodes(left, right)
    );
    const selectedNodeIndexes = new Set([0]);
    const edges = [];

    while (selectedNodeIndexes.size < orderedNodes.length) {
      let bestEdge = null;

      for (const fromIndex of selectedNodeIndexes) {
        const fromNode = orderedNodes[fromIndex];

        for (let toIndex = 0; toIndex < orderedNodes.length; toIndex++) {
          if (selectedNodeIndexes.has(toIndex)) {
            continue;
          }
          ...
          const candidate = {
            fromIndex,
            toIndex,
            distance: ContainerKingdomRenderer.getNetworkNodeDistance(fromNode, toNode),
            fromKey: ContainerKingdomRenderer.getNetworkNodeKey(fromNode),
            toKey: ContainerKingdomRenderer.getNetworkNodeKey(toNode),
          };
          ...
          if (candidate.distance < bestEdge.distance) {
            bestEdge = candidate;
            continue;
          }

          if (candidate.distance === bestEdge.distance) {
            const candidateKey = `${candidate.fromKey}->${candidate.toKey}`;
            const bestKey = `${bestEdge.fromKey}->${bestEdge.toKey}`;
      ...
      selectedNodeIndexes.add(bestEdge.toIndex);
      edges.push({
        from: orderedNodes[bestEdge.fromIndex],
        to: orderedNodes[bestEdge.toIndex],
      });
    }
  ...
  /**
   * @param {{id: string, x: number, y: number}} node
   * @returns {string}
   */
  static getNetworkNodeKey(node) {
    return `${node.x}:${node.y}:${node.id}`;
  }

  /**
   * @param {{id: string, x: number, y: number}} left
   * @param {{id: string, x: number, y: number}} right
   * @returns {number}
   */
  static compareNetworkNodes(left, right) {
    const leftKey = ContainerKingdomRenderer.getNetworkNodeKey(left);
    const rightKey = ContainerKingdomRenderer.getNetworkNodeKey(right);
   ...
   * @returns {number}
   */
  static getNetworkNodeDistance(left, right) {
    return Math.abs(left.x - right.x) + Math.abs(left.y - right.y);
  }

  /**
    * Rebuild the tile chain generated between two cells using the current
    * "horizontal then vertical" routing strategy.
    * @param {Array<{x: number, y: number}>} connectedCells two cells, in order
   * @param {number} cellWidth
   * @param {number} cellHeight
   * @param {number} roadWidth
   * @param {number} roadHeight
