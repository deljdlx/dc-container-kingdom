...
1. Créer la **branche dédiée** depuis `main` (la noter dans `branch:`) ; puis, **sur
   ...
   `doing:`. La branche portera *work* + *verify*. Faire ce travail dans **son
   propre worktree** (branche `<agent>/<slug>`, sans toucher la branche du tree
   principal) — voir [parallel-worktrees](../parallel-worktrees.md).
