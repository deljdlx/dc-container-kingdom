...
type: fix
branch: copilot/fix-fuite-aires-scene-graph
created: 2026-07-26 14:18
ready: 2026-07-26 17:18
doing: 2026-07-26 17:18
verify: 2026-07-26 17:19
done: 2026-07-26 17:19
---
...
- [x] Après `freeArea(x, y)`, l'aire n'est plus dans `board.getChildren()` ni dans
      `board.getAreas()`, et son DOM est détaché.
- [x] La bbox agrégée du board ne croît plus indéfiniment (comportement documenté
      et couvert par un test).
- [x] Test : simuler une marche sur plusieurs aires (piloter `viewport.update(t)`
      à la main) et vérifier que le nombre d'enfants du board reste borné.
- [x] `Board.clear()` remet le board dans un état réellement vide.
- [x] Doc moteur à jour, `npm run verify` vert.
...
- [2026-07-26 17:18] Reprise du ticket sur `copilot/fix-fuite-aires-scene-graph` (worktree dédié recréé).
- [2026-07-26 17:19] Correction `Board.freeArea()` : destruction/détachement réel de l'area du scene-graph + nettoyage de la matrice `areas`.
- [2026-07-26 17:19] Correction `Board.clear()` : vidage complet des children + remise à zéro de `areas`.
- [2026-07-26 17:19] Ajout d'un recalcul des bbox agrégées après retrait d'enfant (`Element.removeChild` + `CollisionSystem.recomputeAggregates`).
- [2026-07-26 17:19] Ajout des tests de non-régression `test/Board.streaming-areas.test.js` (detach, reset clear, bornage streaming, shrink bbox).

### Vérification

- [2026-07-26 17:19] `npm test -- test/Board.streaming-areas.test.js` vert (4/4).
- [2026-07-26 17:19] `npm run verify` vert (26 fichiers, 196 tests).

### Validation

- [2026-07-26 17:19] Validation technique via simulation de marche (`viewport.update(t)` piloté dans le test) : le nombre d'enfants du board reste borné (`<= 81`) sur longue marche.
